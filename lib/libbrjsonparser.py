...
class parser:
	...
	def parseDate(self,day,channel):
		...
		for epg in j['data']['viewer']['allLivestreams']['edges'][0]['node']['epg']:
			...
			if len(publicationOf['essences']['edges']) != 0:
				self.template['metadata']['airedtime'] = broadcastEvent['start'][11:16]
	...
	def parseLinks(self,url):
		...
		for show in j["_embedded"]["teasers"]:
			d = {}
			d['params']['url'] = show["_links"]["self"]["href"]
			d['metadata']['name'] = show["topline"]
			if 'headline' in show:
				d['metadata']['name'] += ' - ' + show['headline']
				d['metadata']['tvshowtitle'] = show['topline']
				
			d['metadata']['subtitle'] = show["topline"]
			d['metadata']['plot'] = show["teaserText"]
			d['metadata']['channel'] = show["channelTitle"]
			duration = show['documentProperties']["br-core:duration"].split(':')
			d['metadata']['duration'] = str(int(duration[0]) * 3600 + int(duration[1]) * 60 + int(duration[2]))
			
			if 'image512' in show["teaserImage"]["_links"]:
				d['metadata']['thumb'] = show["teaserImage"]["_links"]["image512"]["href"]
			elif 'image256' in show["teaserImage"]["_links"]:
				d['metadata']['thumb'] = show["teaserImage"]["_links"]["image256"]["href"]
			try:
				if show['hasSubtitle']:
					d['metadata']['hasSubtitle'] = 'true'
					#d['plot'] += '\n\nUntertitel'
			except:pass
			d['type'] = 'video'
			d['params']['mode'] = 'libBrPlayOld'
